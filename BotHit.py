...
# === CẤU HÌNH TELEGRAM ===
BOT_TOKEN = "8013599424:AAGUg-x2Kn92OtTSH6CIsgKYx2wJCwKRsmU"
...
# Dữ liệu thống kê cầu lớn từ Hitmd5
BIG_STREAK_DATA = {
    "tai": {
...
def send_prediction_update(session):
    ...
    message = (
        f"{EMOJI['diamond']} *HITMD5 VIP - PHÂN TÍCH CHUẨN XÁC* {EMOJI['diamond']}\n"
        f"══════════════════════════\n"
...
def background_task():
    ws_url = "ws://157.10.52.7:10000/ws/hitmd5?id=mrtinhios&key=tinhdeptrai"
    reconnect_delay = 5
    while True:
        try:
                ...
                if "message" in update:
                    ...
                    if text:
                        if text.startswith("/start"):
                            welcome_message = (
                                f"{EMOJI['diamond']} *HITMD5 VIP - CHÀO MỪNG BẠN* {EMOJI['diamond']}\n"
                                f"══════════════════════════\n"
                            ...
                            buttons = [
                                ...
                                [{"text": f"{EMOJI['money_bag']} Liên hệ mua key", "url": "https://t.me/mrtinhios"}]
                            ]
                    ...
                    if callback_data.startswith("broadcast_confirm:"):
                        ...
                        help_activate_message = (
                            ...
                            f"{EMOJI['team']} Liên hệ: @mrtinhios để được hỗ trợ"
                        )
...
def main():
    ...
    if c.fetchone()[0] == 0:
        print(f"{EMOJI['admin']} Thêm admin đầu tiên với ID: 6463176046 và 6463176046")
        c.execute("INSERT OR IGNORE INTO admins (chat_id) VALUES (?)", (6463176046,))
        c.execute("INSERT OR IGNORE INTO admins (chat_id) VALUES (?)", (6463176046,))
        conn.commit()
    conn.close()

    print(f"\n{EMOJI['diamond']} {'*'*20} {EMOJI['diamond']}")
    print(f"{EMOJI['rocket']} *HITMD5 VIP - BOT TÀI XỈU CHUẨN XÁC* {EMOJI['rocket']}")
